...
export function Navbar() {
  ...
  return (
    <motion.header
      ...
      <nav
        className={`mx-auto flex max-w-container items-center justify-between gap-4 px-4 transition-all duration-300 sm:px-6 ${
        scrolled ? 'rounded-btn glass shadow-glass mx-4 py-2.5' : 'py-2'}`
        }>
        
        <Link to="/" className="flex items-center gap-2.5" aria-label={CLINIC.name}>
          <span className="grid h-10 w-10 place-items-center rounded-btn bg-brand-primary text-white shadow-float">
            <EyeIcon className="h-5 w-5 animate-blink" />
          </span>
          <span className="hidden flex-col leading-none sm:flex">
            <span className="font-display text-base font-bold text-brand-dark">Renu Eye Care</span>
            <span className="text-[10px] font-medium uppercase tracking-widest text-brand-primary">
              {CLINIC.tagline}
            </span>
          </span>
        </Link>
        ...
        <motion.div
          ...
          className="mx-4 mt-2 overflow-hidden rounded-card glass shadow-glass lg:hidden">
            ...
            <NavLink
              ...
              to={n.to}
              className={({ isActive }) =>
              `rounded-btn px-4 py-3 text-sm font-medium ${
              isActive ? 'bg-brand-section text-brand-primary' : 'text-brand-dark'}`
